...
const ExcalidrawWrapper = () => {
  ...
  // NoemAI: Expose excalidrawAPI on window for external programmatic access
  // Including custom helper methods for easier image insertion with collaboration support
  useEffect(() => {
    if (excalidrawAPI) {
      // ═══════════════════════════════════════════════════════════════════════
      // HELPER: Create image element with proper collaboration-ready structure
      // Uses status: "pending" to trigger automatic file upload via FileManager
      // IMPORTANT: version must be high enough to pass broadcastElements check:
      //   getSceneVersion(elements) > lastBroadcastedOrReceivedSceneVersion
      // ═══════════════════════════════════════════════════════════════════════
      const createImageElement = (params: {
        elementId: string;
        fileId: FileId;
        x: number;
        y: number;
        ...
        // CRITICAL: Use "pending" to enable collaboration sync via Portal.queueFileUpload
        // The FileManager will update this to "saved" after successful storage upload
      ...
      const calculateDimensions = (
        ...
        let width = requestedWidth || naturalWidth;
        let height = requestedHeight || naturalHeight;
      ...
      // ═══════════════════════════════════════════════════════════════════════
      const loadImageDimensions = async (dataUrl: string): Promise<{ naturalWidth: number; naturalHeight: number }> => {
        const img = new Image();
      ...
      // ═══════════════════════════════════════════════════════════════════════
      // HELPER: Trigger collaboration sync if in collaborative mode
      // ═══════════════════════════════════════════════════════════════════════
      const triggerCollabSync = () => {
        if (collabAPI?.isCollaborating()) {
          const elements = excalidrawAPI.getSceneElements();
          collabAPI.syncElements(elements);
          console.log("[NoemAI] Collaboration sync triggered");
        }
      };

      // ═══════════════════════════════════════════════════════════════════════
      // PUBLIC API: Add image from URL (fetches and converts to DataURL)
      // ═══════════════════════════════════════════════════════════════════════
      const addImageFromUrl = async (
        url: string,
        x: number,
        y: number,
        options?: { width?: number; height?: number; id?: string }
      ): Promise<{ fileId: string; elementId: string }> => {
        // 1. Fetch the image
        ...
        // 6. Create and add image element (with status: "pending" for collab sync)
        const imageElement = createImageElement({ elementId, fileId, x, y, width, height });
        const currentElements = excalidrawAPI.getSceneElements();
        excalidrawAPI.updateScene({
          elements: [...currentElements, imageElement],
        });

        // 7. COLLABORATION SYNC FIX: Force immediate file upload and broadcast
        if (collabAPI?.isCollaborating()) {
          console.log(`[NoemAI] Collaboration mode: forcing immediate file upload for ${fileId}`);
          ...
          const waitForUpload = async (maxWaitMs = 5000): Promise<boolean> => {
            const startTime = Date.now();
            return new Promise((resolve) => {
              const checkStatus = () => {
                const elements = excalidrawAPI.getSceneElementsIncludingDeleted();
                const element = elements.find(el => el.id === elementId);
                if (element && 'status' in element && element.status === 'saved') {
                  console.log(`[NoemAI] File uploaded successfully, status: saved`);
                  resolve(true);
                } else if (Date.now() - startTime > maxWaitMs) {
                  console.log(`[NoemAI] Upload timeout, proceeding with broadcast anyway`);
                  resolve(false);
                } else {
          ...
          if (queueBroadcast?.flush) {
            ...
            console.log(`[NoemAI] Forced full scene broadcast`);
          }
        } else {
          triggerCollabSync();
        }

        console.log(`[NoemAI] Image added from URL: fileId=${fileId}, elementId=${elementId}, size=${width}x${height}`);
        return { fileId, elementId };
      };

      // ═══════════════════════════════════════════════════════════════════════
      // PUBLIC API: Add image from DataURL directly
      // ═══════════════════════════════════════════════════════════════════════
      const addImageFromDataUrl = async (
        ...
        // 4. Create and add image element (with status: "pending" for collab sync)
        const imageElement = createImageElement({ elementId, fileId, x, y, width, height });
        const currentElements = excalidrawAPI.getSceneElements();
        excalidrawAPI.updateScene({
          elements: [...currentElements, imageElement],
        ...
        // 5. COLLABORATION SYNC FIX: Force immediate file upload and broadcast
        // The issue: queueFileUpload is throttled (300ms), and status change to "saved"
        // uses CaptureUpdateAction.NEVER, so the change isn't broadcasted until the next
        // full sync (20 seconds later). This forces immediate upload and broadcast.
        if (collabAPI?.isCollaborating()) {
          console.log(`[NoemAI] Collaboration mode: forcing immediate file upload for ${fileId}`);
          ...
          // Force a full scene broadcast to ensure status="saved" is sent to collaborators
          const queueBroadcast = (collabAPI as any).queueBroadcastAllElements;
          if (queueBroadcast?.flush) {
            queueBroadcast.flush();
            console.log(`[NoemAI] Forced full scene broadcast`);
          ...
          // Not in collaboration mode, just do normal sync
          triggerCollabSync();
        }

        console.log(`[NoemAI] Image added from DataURL: fileId=${fileId}, elementId=${elementId}, size=${width}x${height}`);
        return { fileId, elementId };
      };

      // ═══════════════════════════════════════════════════════════════════════
      // Expose the API on window for external programmatic access
      // ═══════════════════════════════════════════════════════════════════════
      (window as any).excalidrawAPI = {
        ...
        syncElements: () => triggerCollabSync(),
        // Direct access to collabAPI for full control
        getCollabAPI: () => collabAPI,
      };

      // Expose Jotai store and atoms for advanced debugging and state access
      (window as any).appJotaiStore = appJotaiStore;
      (window as any).collabAtoms = {
        isCollaboratingAtom,
        collabAPIAtom,
        isOfflineAtom,
      };

      console.log("[NoemAI] excalidrawAPI exposed with collaboration-aware helpers");
      console.log("[NoemAI] collabAPI available:", !!collabAPI, "isCollaborating:", collabAPI?.isCollaborating());
      console.log("[NoemAI] Jotai store exposed as window.appJotaiStore, atoms as window.collabAtoms");
    }
    return () => {
